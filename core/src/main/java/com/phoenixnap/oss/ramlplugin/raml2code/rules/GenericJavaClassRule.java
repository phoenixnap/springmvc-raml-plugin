...
import com.phoenixnap.oss.ramlplugin.raml2code.helpers.CodeModelHelper.JExtMethod;
...
/**
 * This is a configurable template rule which generates a Java artefact (class
 * or interface) based on an ApiControllerMetadata instance. The code generation
 * steps are executed by applying subsequent Rule instances.
 *
 * The caller has to set appropriate Rule instances for each part of the Java
 * class generation.
 *
 * At least a packageRule and a classRule is mandatory. If the
 * ApiControllerMetadata contains one or more endpoints (ApiMappingMetadata) at
 * least methodSignatureRule is mandatory.
 *
 * packageRule - a package declaration rule classCommentRule - an optional class
 * comment rule classAnnotationRules - a set of class annotation rules classRule
 * - a class declaration rule implementsExtendsRule - an optional rule to define
 * implements/extends part of the class declaration fieldDeclerationRules - a
 * set of field declaration rules. May be empty. methodCommentRule - an optional
 * method comment rule methodAnnotationRules - a set of method annotation rules.
 * May be empty. methodSignatureRule - a method signature rule (just the
 * signature, no annotations, no body) metodBodyRule - an optional method body
 * rule
 *
