...
public class SchemaHelper {
	 ...
	 * @param rawType
	 *            RAML type
	 * @return The Java Class which maps to this Simple RAML ParamType or string if
	 *         one is not found
	 */
	...
	/**
	 * Extracts the name from a schema in this order of precedence: 1. If the schema
	 * contains an ID element 2. The name of the schema within the RAML document 3.
	 * The autogenerated name based on the enclosing method
	 *
	 * @param schema
				...
				if (ref != null) {
					...
					try {
						schema = IOUtils.toString(
								Thread.currentThread().getContextClassLoader().getResourceAsStream(ref), "UTF-8");
	...
	/**
	 * Maps a JSON Schema to a JCodeModel using JSONSchema2Pojo and encapsulates it
	 * along with some metadata into an {@link ApiBodyMetadata} object.
	 *
	 * @param document
	 ...
	 * @param name
	 *            The suggested name of the class based on the api call and whether
	 *            it's a request/response. This will only be used if no suitable
	 *            alternative is found in the schema
	 * @param schemaLocation
	 *            Base location of this schema, will be used to create absolute URIs
	 *            for $ref tags eg "classpath:/"
	...
	public static ApiBodyMetadata mapSchemaToPojo(RamlRoot document, String schema, String basePackage, String name,
		...
		JCodeModel codeModel = buildBodyJCodeModel(basePackage,
				StringUtils.hasText(schemaLocation) ? schemaLocation : "classpath:/", resolvedName, resolvedSchema,
				null);
		if (codeModel != null) {
	 ...
	 * @param schemaLocation
	 *            The location of this schema, will be used to create absolute URIs
	 *            for $ref tags eg "classpath:/"
	 * @param name
	 *            The class name
	 * @param schema
	...
	public static JCodeModel buildBodyJCodeModel(String basePackage, String schemaLocation, String name, String schema,
		...
		try {
			...
			if (useParent && e.getMessage().contains("classpath")) {
				logger.debug(
						"Referenced Schema contains self $refs or not found in classpath. Regenerating model withouth classpath: for "
								+ name);
	...
	/**
	 * Returns a configuration for the JSON Schema 2 POJO that is in line with the
	 * defaults used in the plugin so far
	 *
	 ...
	 * parameters are supplied null, it will use the value defined in the default
	 * configuration
	 *
