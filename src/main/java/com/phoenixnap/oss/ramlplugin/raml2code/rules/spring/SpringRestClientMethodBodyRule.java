 ...
 * /{id} get:
 *
 * OUTPUT: HttpEntity httpEntity = new HttpEntity(); return
 * this.restTemplate.exchange(/, GET, httpEntity,responseBody);
 *
...
public class SpringRestClientMethodBodyRule implements Rule<CodeModelHelper.JExtMethod, JMethod, ApiActionMetadata> {
	...
	public JMethod apply(ApiActionMetadata endpointMetadata, CodeModelHelper.JExtMethod generatableType) {
		...
		// If we have response bodies defined, we need to add them to our
		// accepts headers list
		...
		// Iterate over Response Bodies and add each distinct mime type to
		// accepts headers
		...
		// Get the parameters from the model and put them in a map for easy
		// lookup
		...
		// get all uri params from metadata set and add them to the param map in
		// code
		...
		if (!CollectionUtils.isEmpty(returnType.getTypeParameters())) {
			// if yes - build the parameterized type reference and change
			// returnExpression
			// Due to issue 61, it is generated as
			// class _P extends
			// org.springframework.core.ParameterizedTypeReference<java.util.List<java.lang.String>>
